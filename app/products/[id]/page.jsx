...
import { Star, Award, Shield, Truck, CheckCircle, AlertCircle } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-2">محصول پیدا نشد</h1>
    ...
    return (
      <div className="space-y-6">
        ...
        {product.electricalParameters.stc && (
          <div>
            <h4 className="text-lg font-semibold mb-4 text-blue-600">پارامترهای STC</h4>
        ...
        {product.electricalParameters.nmot && (
          <div>
            <h4 className="text-lg font-semibold mb-4 text-green-600">پارامترهای NMOT</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(product.electricalParameters.nmot).map(([key, value]) => (
    ...
    return (
      <div className="overflow-x-auto">
        <table className="w-full border-collapse border border-gray-300">
          <thead className="bg-gray-50">
            <tr>
              <th className="border border-gray-300 px-4 py-2 text-right">مدل</th>
              <th className="border border-gray-300 px-4 py-2 text-right">قدرت خروجی</th>
              <th className="border border-gray-300 px-4 py-2 text-right">حداکثر قدرت ظاهری</th>
              <th className="border border-gray-300 px-4 py-2 text-right">جریان خروجی</th>
            </tr>
          </thead>
          <tbody>
            {product.models.map((model, index) => (
              <tr key={index} className="hover:bg-gray-50">
                ...
                <td className="border border-gray-300 px-4 py-2">{model.ratedACOutputActivePower || model.ratedPower}</td>
                <td className="border border-gray-300 px-4 py-2">{model.maxACOutputApparentPower || model.maxACApparentPower}</td>
                <td className="border border-gray-300 px-4 py-2">{model.ratedACOutputCurrent || model.ratedACCurrent}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            {product.temperatureCharacteristics && renderSection(
              "مشخصات دمایی",
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(product.temperatureCharacteristics).map(([key, value]) => (
                  <div key={key} className="flex justify-between items-center py-2 border-b border-gray-100">
                    <span className="text-gray-600">{key}</span>
                    <span className="text-gray-900 font-medium">{value}</span>
                  </div>
            ...
            {/* Maximum Ratings */}
            {product.maximumRatings && renderSection(
              "حداکثر مقادیر",
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(product.maximumRatings).map(([key, value]) => (
                  <div key={key} className="flex justify-between items-center py-2 border-b border-gray-100">
                    <span className="text-gray-600">{key}</span>
                    <span className="text-gray-900 font-medium">{value}</span>
                  </div>
          ...
          <div className="space-y-6">
            {/* Features */}
            {product.features && renderSection(
              "ویژگی‌های کلیدی",
            ...
            {/* Protection Features */}
            {product.protectionFeatures && renderSection(
              "ویژگی‌های حفاظتی",
            ...
            {/* Certifications */}
            {product.certifications && renderSection(
              "گواهینامه‌ها",
              <div className="flex flex-wrap gap-2">
            ...
            {/* Warranty */}
            {product.warranty && renderSection(
              "گارانتی",
            ...
            {/* Communication */}
            {product.communication && renderSection(
              "ارتباطات",
              <div className="space-y-3">
            ...
            {/* Manufacturer */}
            {product.manufacturer && renderSection(
              "اطلاعات سازنده",
              <div className="space-y-3">
